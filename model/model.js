...
export default class Model {
    ...
    for (let row = 0; row < this.rows; row++) {
      for (let col = 0; col < this.cols; col++) {
        if (this.grid[row][col].tileType.BOMB == false) {
          ...
          switch (count) {
            case 0:
              this.grid[row][col].tileType.ZERO = true;
              break;
            case 1:
              this.grid[row][col].tileType.ONE = true;
              break;
            case 2:
              this.grid[row][col].tileType.TWO = true;
              break;
            case 3:
              this.grid[row][col].tileType.THREE = true;
              break;
            case 4:
              this.grid[row][col].tileType.FOUR = true;
              break;
            case 5:
              this.grid[row][col].tileType.FIVE = true;
              break;
            case 6:
              this.grid[row][col].tileType.SIX = true;
              break;
            case 7:
              this.grid[row][col].tileType.SEVEN = true;
              break;
            case 8:
              this.grid[row][col].tileType.EIGHT = true;
              break;
          }
        }
      }
  ...
  countNeighborBombs(r, c) {
    ...
    if (r + 1 < this.rows &&
      c + 1 < this.cols &&
      this.grid[r + 1][c + 1].tileType.BOMB
    )
      count++; // downRight
    if (r - 1 >= 0 && c - 1 >= 0 && this.grid[r - 1][c - 1].tileType.BOMB)
      count++; //  up left
    if (
      r + 1 < this.rows &&
      c - 1 >= 0 &&
